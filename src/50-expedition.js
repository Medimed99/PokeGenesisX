...
const NODE_TYPES = {
  fight:   {n:"Sauvage",    i:"battle", c:"var(--ink)"},
  trainer: {n:"Archiviste", i:"user",   c:"var(--cyan)"},
  elite:   {n:"Élite",      i:"boss",   c:"var(--magenta)"},
  draft:   {n:"Recrutement",i:"heart",  c:"var(--green)"},
  swap:    {n:"Échange",    i:"refresh",c:"var(--violet)"},
...
/* ============================================================
   BORNES DE PUISSANCE
   Le niveau seul ne suffit pas a contenir la difficulte : un
   Pokemon a 600 de total de statistiques ecrase un etage entier
   meme au meme niveau. Chaque etage tire donc dans une fourchette
   de total, ce qui rend la courbe de puissance lisible et empeche
   de trouver un monstre a l'etage deux.
   ============================================================ */
const BST_BANDS = [
  [200, 320], [230, 350], [270, 390], [300, 420],
  [330, 450], [360, 470], [390, 490], [410, 510],
  [430, 530], [450, 545], [470, 560], [500, 999]
];
function bstBand(floor){
  return BST_BANDS[clamp(floor, 0, BST_BANDS.length - 1)];
}
/* tirage d'une espece sauvage bornee par l'etage */
function wildIdFor(floor, opts){
  opts = opts || {};
  const [lo, hi] = bstBand(floor);
  const pool = [];
  for(const r of unlockedRegions())
    for(let i = r.from; i <= r.to; i++){
      ...
      if(opts.type && !p.types.includes(opts.type)) continue;
      pool.push(i);
    }
  if(pool.length) return pick(pool);
  /* fourchette vide (type rare a cet etage) : on relache la contrainte de type */
  return opts.type ? wildIdFor(floor) : randomWildId();
}

/* ============================================================
   POIDS DE NŒUDS PAR COUCHE
   Le debut de parcours doit servir a batir l'equipe, la fin a la
   mettre a l'epreuve. Des poids constants du premier au dernier
   etage donnent un parcours sans arc.
   ============================================================ */
const NODE_W = [
  /* étage 0 : on part toujours au contact */
  null,
  {fight:30, draft:34, event:10, shop:12, treasure:14},
  {fight:26, draft:26, trainer:16, event:12, shop:10, treasure:10},
  {fight:20, draft:18, trainer:22, elite:8,  event:14, rest:8,  shop:6,  swap:4},
  {fight:18, draft:14, trainer:24, elite:10, event:14, rest:8,  shop:6,  swap:6},
  {fight:16, draft:12, trainer:24, elite:12, event:15, rest:9,  shop:6,  swap:6},
  {fight:14, draft:10, trainer:24, elite:14, event:16, rest:10, shop:6,  swap:6},
  {fight:13, draft:8,  trainer:24, elite:16, event:16, rest:11, shop:6,  swap:6},
  {fight:12, draft:7,  trainer:23, elite:18, event:16, rest:12, shop:6,  swap:6},
  {fight:12, draft:6,  trainer:22, elite:20, event:16, rest:12, shop:6,  swap:6},
  {fight:12, draft:5,  trainer:20, elite:22, event:16, rest:13, shop:6,  swap:6}
];
...
function randomWildId(){
  ...
  const r = pick(regs);
  for(let k=0;k<40;k++){ const id = randInt(r.from, r.to); if(POKE[id] && !POKE[id].leg && !isExclusive(id)) return id; }
  return r.from;
}
function newRunMon(run, id, lvl){
  ...
  applyRelics(run, f);
  return {id, level, hp:f.maxHp, maxHp:f.maxHp, item:null};
}
...
/* Un membre tombe ne revient pas. Aucun soin, aucun repos, aucune relique de
   confort ne le releve : seule la Reindexation automatique le peut, et elle est
   rare. C'est ce qui donne du poids a chaque choix de route. */
function healTeam(run, pct){
  for(const m of run.team){
    if(m.hp <= 0) continue;
    m.hp = Math.min(m.maxHp, m.hp + Math.ceil(m.maxHp*pct));
  }
}
function fallen(run){ return run.team.filter(m=>m.hp <= 0); }
...
/* ============================================================
   GÉNÉRATION DE CARTE
   L'ancienne version reliait chaque noeud au plus proche en
   colonne, ce qui produisait des chemins entrecroises : le joueur
   ne voyait plus ou menait sa route.

   Ici les liaisons sont MONOTONES. Chaque noeud d'une rangee
   recoit un bloc contigu de cibles dans la rangee suivante, et
   les blocs se suivent de gauche a droite. Deux liaisons ne
   peuvent donc jamais se croiser, et la carte reste lisible
   meme avec quatre colonnes.
   ============================================================ */
function genMap(seed){
  ...
  for(let y = 0; y < EXP_ROWS; y++){
    ...
    else if(y === EXP_ROWS - 2)  cols = [1, 2];
    else {
      const n = 2 + Math.floor(r() * 3);           /* 2 a 4 noeuds */
      cols = shuffle([0, 1, 2, 3], r).slice(0, n).sort((a2, b2) => a2 - b2);
    }
    rows.push(cols.map(c => ({c, t: nodeTypeFor(y, r), next: [], done: false})));
  }

  for(let y = 0; y < EXP_ROWS - 1; y++){
    const cur = rows[y], nx = rows[y + 1];
    const n = cur.length, m = nx.length;

    /* 1. partition en blocs contigus : toute la rangee suivante est couverte */
    const lo = [], hi = [];
    for(let i = 0; i < n; i++){
      lo[i] = Math.floor(i * m / n);
      hi[i] = Math.max(lo[i], Math.ceil((i + 1) * m / n) - 1);
    }

    /* 2. embranchement, uniquement la ou il ne peut rien croiser :
       on ne deborde d'un cran que si le bloc suivant commence apres le notre */
    for(let i = 0; i < n; i++){
      const nextLo = (i + 1 < n) ? lo[i + 1] : m;
      if(hi[i] + 1 < m && nextLo > hi[i] && r() < 0.5) hi[i] = hi[i] + 1;
    }

    /* 3. ecriture des liaisons */
    for(let i = 0; i < n; i++){
      const t = [];
      for(let j = lo[i]; j <= hi[i]; j++) t.push(j);
      cur[i].next = t;
    }
  }
...
function nodeTypeFor(y, r){
  ...
  if(y === 0) return "fight";
  if(y === EXP_ROWS - 2) return "rest";      /* repos garanti avant le noyau */
  const w = NODE_W[Math.min(y, NODE_W.length - 1)] || NODE_W[NODE_W.length - 1];
  const mods = expMods();
  const tot = Object.entries(w).reduce((a2, [k, v]) => a2 + v * (mods[k] !== undefined ? mods[k] : 1), 0);
  let x = r() * tot;
  for(const [k, v] of Object.entries(w)){
    x -= v * (mods[k] !== undefined ? mods[k] : 1);
    if(x <= 0) return k;
  }
  return "fight";
}
/* le mode de parcours modifie les poids : en Nuzlocke, on ne recrute plus */
function expMods(){
  const m = (S.expedition && S.expedition.mode) || S.expMode || "classic";
  if(m === "nuzlocke") return {draft:0, swap:0, rest:0.6};
  return {};
}
...
function startExpedition(ids){
  ...
  const run = {
    ...
    revives: 1,          /* une seule remise en ligne par parcours : un choix, pas un filet */
    mode: S.expMode || "classic", lost: 0, itemDraft: null, draft: null,
...
function endExpedition(win){
  ...
  if(win){
    ...
    pendingBooster = [{s:cw.s, id:cw.id, fresh:cwNew}];
    if(rng() < 0.5){
      const h = randomHeld();
      if(h && grantHeld(h))
  ...
  hallRecord(run, win);
  if(win) factionPts(20);
  S.expedition = null;
  save();

  const relicList = run.relics.map(id=>RELICS.find(x=>x.id===id)).filter(Boolean);
  /* le sachet s'ouvre apres le recapitulatif : une chose a la fois */
  sheet(`<div class="center exprecap">
    ...
    ${gains.filter(g=>!g.alive).length?`
      <div class="h sm" style="margin-top:11px">DÉSINDEXÉS</div>
      <div class="tiny muted" style="margin-bottom:6px">Ils restent intacts dans votre archive,
        mais n'ont rien gagné de ce parcours.</div>
      <div class="wrap" style="justify-content:center">
        ${gains.filter(g=>!g.alive).map(g=>`<span class="fallenchip">
          <span class="sprbox" style="width:22px;height:22px">${sprite(g.id, false, "ghosted")}</span>
          ${esc(POKE[g.id].name)}</span>`).join("")}
      </div>`:""}
    ...
    <button class="btn pri wide" style="margin-top:12px" data-act="expdone">
      ${(pendingEgg || pendingBooster) ? "Examiner le butin" : "Retour"}</button>
  </div>`);
  checkAchievements();
  guideTick();
}
/* le butin se deroule un objet a la fois : oeuf, puis sachet, puis retour */
function expLootChain(){
  if(pendingEgg){
    const t = pendingEgg; pendingEgg = null;
    if(showEggFind(t, expLootChain)) return;
  }
  if(pendingBooster){
    const cards = pendingBooster; pendingBooster = null;
    if(showBooster(cards, {title:"BUTIN DU NOYAU",
        sub:"Scellé au fond de la couche. Le système n'a jamais su ce qu'il contenait.",
        onDone:expLootChain})) return;
  }
  go("expedition");
  checkStoryTriggers();
}
ACTIONS.expdone = () => { closeSheet(); expLootChain(); };
...
SCREENS.expedition = {
  html(){
    ...
    return `
      ...
      <div class="row between tiny" style="margin-top:5px">
        <span class="dim">Étage ${floor}/${EXP_ROWS}</span>
        <span class="${fallen(run).length?"bad":"dim"}">
          ${run.team.filter(m=>m.hp>0).length}/${run.team.length} en ligne${
          fallen(run).length?` · ${fallen(run).length} perdu(s)`:""}</span>
        <span class="dim">${run.relics.length} relique(s)</span>
      </div>
      ...
      <div class="btn-grid c2" style="margin-top:9px">
        ...
        <button class="btn sm ${(run.revives>0&&fallen(run).length)?"dan":""}" data-act="exprevive"
          ${(run.revives>0&&fallen(run).length)?"":"disabled"}>
          Réindexer (${run.revives||0})</button>
      </div>
      <div class="btn-grid c3" style="margin-top:7px">
        <button class="btn sm" data-act="exporder">${ic("grid")} Ordre</button>
  ...
  lobby(){
    ...
    return `
      ...
      ${moduleGoal("Douze étages, une route que l'on ne refait pas. Les points de vie ne remontent jamais complètement, et <b>un membre tombé est perdu pour le reste du parcours</b>.",
        "Des niveaux de combat permanents pour les survivants, des œufs, et des cartes de collection.")}

      <div class="h sm">COMMENT DESCENDRE</div>
      <div class="modegrid c3">
        ${Object.entries(EXP_MODES).map(([k,md])=>`
          <div class="modecard ${(!md.go && expMode()===k)?"on":""} ${md.go?"dest":""}"
            data-act="expsetmode" data-m="${k}" style="--mc:${md.c}">
            <div class="mc-n">${esc(md.n)}</div>
            <div class="mc-d">${esc(md.d)}</div>
            ${md.mult>1?`<div class="mc-x">récompenses ×${md.mult}</div>`:""}
            ${md.go?`<div class="mc-x" style="color:var(--violet)">record : étage ${(S.tower&&S.tower.best)||0}</div>`:""}
          </div>`).join("")}
      </div>
...
ACTIONS.expstart = () => {
  ...
  teamPicker("Équipe de départ (5 max)", 5, ids=>{ if(ids.length) startExpedition(ids); });
};
ACTIONS.expabandon = () => {
...
/* la seule facon de relever un membre tombe : une fois, et il faut choisir qui */
ACTIONS.exprevive = () => {
  const run = S.expedition;
  const down = fallen(run);
  if(!run.revives || !down.length) return;
  sheet(`${sheetHead("Réindexer un membre")}
    <div class="tiny muted" style="margin-bottom:9px">Une seule réindexation par parcours.
      Le membre revient à la moitié de ses points de vie. Ce choix est définitif.</div>
    <div class="tp-sel">
      ${down.map(m=>{
        const i = run.team.indexOf(m);
        return `<div class="tp-row">
          <span class="sprbox" style="width:38px;height:38px">${sprite(m.id, S.dex[m.id]?.shiny, "ghosted")}</span>
          <div class="grow"><div class="tiny">${esc(POKE[m.id].name)}
            <span class="dim">N.${m.level}</span></div>
            <div class="tiny bad">désindexé</div></div>
          <button class="btn xs pri" data-act="exprevivedo" data-i="${i}">Réindexer</button>
        </div>`;}).join("")}
    </div>
    <button class="btn ghost wide" style="margin-top:10px" data-act="closeandrefresh">Annuler</button>`);
};
ACTIONS.exprevivedo = d => {
  const run = S.expedition, m = run.team[+d.i];
  if(!m || m.hp > 0 || !run.revives) return;
  run.revives--;
  m.hp = Math.floor(m.maxHp * 0.5);
  Sfx.win(); buzz([20,40,60]);
  toast(POKE[m.id].name + " revient en ligne", "", "heart");
...
function enterNode(run, node){
  switch(node.t){
    ...
    case "trainer": run.trainer = null; return expFight(run, "trainer");
    case "draft":    return expDraft(run);
    case "swap":     return expSwapNode(run);
    case "rest":     return expRest(run);
    case "shop":     return expShop(run);
    case "treasure": return expItemNode(run);
    case "event":    return expEvent(run);
  }
}

/* ============================================================
   RECRUTEMENT PAR TIRAGE
   Un seul candidat, c'est une recompense ; trois candidats, c'est
   une decision. Toute la tension du genre est la.
   ============================================================ */
function draftPool(run, n, lvBonus){
  const out = [], seen = new Set();
  let guard = 0;
  while(out.length < n && guard++ < 60){
    const id = wildIdFor(run.floor);
    if(seen.has(id)) continue;
    seen.add(id);
    out.push({id, lvl: expEnemyLevel(run, (lvBonus||0) + 1)});
  }
  return out;
}
function draftCard(c, action, extra){
  const p = POKE[c.id];
  return `<div class="draftcard" data-act="${action}" data-id="${c.id}" data-lvl="${c.lvl}">
    <span class="sprbox" style="width:62px;height:62px">${sprite(c.id, false, "")}</span>
    <div class="dc-n">${esc(p.name)}</div>
    <div class="wrap" style="justify-content:center;gap:2px">${typeTags(p.types)}</div>
    <div class="dc-s"><b>N.${c.lvl}</b> · total ${p.bst}</div>
    ${extra || ""}
  </div>`;
}
function expDraft(run){
  run.draft = draftPool(run, 3, 0);
  save();
  const full = run.team.length >= 6;
  sheet(`${sheetHead("Trois signatures détectées")}
    <div class="tiny muted" style="margin-bottom:10px">
      ${full ? "Votre équipe est complète : en recruter un reviendra à en remplacer un autre."
             : "Une seule peut rejoindre l'expédition. Les deux autres repartent."}</div>
    <div class="draftrow">
      ${run.draft.map(c=>draftCard(c, "expdraftpick")).join("")}
    </div>
    <button class="btn ghost wide" style="margin-top:11px" data-act="expdraftskip">
      Passer · +${80 + run.floor*8} Bits</button>`);
}
ACTIONS.expdraftskip = () => {
  const run = S.expedition;
  run.bits += 80 + run.floor*8;
  run.draft = null;
  toast(`+${80 + run.floor*8} Bits`, "", "coin");
  save(); closeSheet(); go("expedition");
};
ACTIONS.expdraftpick = d => {
  const run = S.expedition;
  const id = +d.id, lvl = +d.lvl;
  if(run.team.length < 6){
    run.team.push(newRunMon(run, id, lvl));
    run.draft = null;
    toast(POKE[id].name + " rejoint l'expédition", "", "check");
    save(); closeSheet(); go("expedition");
    return;
  }
  /* equipe pleine : il faut choisir qui part */
  sheet(`${sheetHead("Remplacer qui ?")}
    <div class="tiny muted" style="margin-bottom:9px">Le membre remplacé quitte l'expédition.
      Il ne gagnera aucun niveau permanent, mais reste intact dans votre archive.</div>
    <div class="tp-sel">
      ${run.team.map((m,i)=>{
        ...
        return `<div class="tp-row">
          <span class="sprbox" style="width:38px;height:38px">${sprite(m.id, S.dex[m.id]?.shiny, "")}</span>
          <div class="grow"><div class="tiny">${esc(POKE[m.id].name)}
            <span class="dim">N.${m.level} · total ${POKE[m.id].bst}</span>
            ${leg?'<span class="tt" style="color:var(--amber);border-color:var(--amber)">LÉG.</span>':""}</div>
            <div class="tiny ${m.hp>0?"dim":"bad"}">${m.hp>0?`${m.hp} / ${m.maxHp} PV`:"hors ligne"}</div></div>
          <button class="btn xs ${leg?"dan":""}" data-act="expdraftswap"
            data-i="${i}" data-id="${id}" data-lvl="${lvl}">Remplacer</button>
        </div>`;}).join("")}
    </div>
    <button class="btn ghost wide" style="margin-top:10px" data-act="expdraftskip">
      Finalement, passer</button>`);
};
ACTIONS.expdraftswap = d => {
  const run = S.expedition, i = +d.i;
  const out = run.team[i];
  run.team[i] = newRunMon(run, +d.id, +d.lvl);
  run.draft = null;
  toast(`${POKE[out.id].name} quitte l'expédition`, "warn", "refresh");
  save(); closeSheet(); go("expedition");
};

/* ---------- nœud Échange : céder un membre contre trois niveaux ---------- */
function expSwapNode(run){
  run.draft = draftPool(run, 3, 3);
  save();
  sheet(`${sheetHead("Terminal d'échange")}
    <div class="tiny muted" style="margin-bottom:10px">Le terminal accepte un de vos Pokémon et
      en rend un autre, <b>trois niveaux au-dessus</b>. L'opération est irréversible, et vous ne
      choisissez pas ce qui revient une fois l'échange lancé.</div>
    <div class="draftrow">
      ${run.draft.map(c=>draftCard(c, "expswapoffer")).join("")}
    </div>
    <button class="btn ghost wide" style="margin-top:11px" data-act="expdraftskip">
      Ne rien échanger · +${60 + run.floor*6} Bits</button>`);
}
ACTIONS.expswapoffer = d => {
  const run = S.expedition, id = +d.id, lvl = +d.lvl;
  sheet(`${sheetHead("Céder qui ?")}
    <div class="tiny muted" style="margin-bottom:9px">En échange de
      <b>${esc(POKE[id].name)}</b> niveau ${lvl}.</div>
    <div class="tp-sel">
      ${run.team.map((m,i)=>`<div class="tp-row">
        <span class="sprbox" style="width:38px;height:38px">${sprite(m.id, S.dex[m.id]?.shiny, "")}</span>
        <div class="grow"><div class="tiny">${esc(POKE[m.id].name)}
          <span class="dim">N.${m.level} · total ${POKE[m.id].bst}</span></div></div>
        <button class="btn xs" data-act="expdraftswap" data-i="${i}" data-id="${id}" data-lvl="${lvl}">
          Céder</button>
      </div>`).join("")}
    </div>
    <button class="btn ghost wide" style="margin-top:10px" data-act="expdraftskip">Annuler</button>`);
};
...
/* Noms d'archivistes adverses : chacun est specialise dans un type,
   ce qui rend le noeud lisible avant d'y entrer — on sait contre quoi
   on va se battre, donc on peut s'y preparer. */
const TRAINERS = [
  {n:"Veilleur de strate",  t:7},  {n:"Sonde de surface",    t:11},
  {n:"Archiviste de fer",   t:9},  {n:"Compilateur",         t:13},
  {n:"Jardinier d'index",   t:12}, {n:"Fondeur",             t:10},
  {n:"Cartographe",         t:5},  {n:"Lecteur de mémoire",  t:14},
  {n:"Gardien de cache",    t:6},  {n:"Purgeur",             t:4},
  {n:"Analyste",            t:1},  {n:"Correcteur",          t:2},
  {n:"Veille de nuit",      t:17}, {n:"Trieur de flux",      t:3},
  {n:"Archiviste gelé",     t:15}, {n:"Scribe spectral",     t:8},
  {n:"Copiste draconique",  t:16}, {n:"Relecteur",           t:18}
];
function trainerFor(run){
  if(!run.trainer) run.trainer = pick(TRAINERS);
  return run.trainer;
}

/* Le niveau adverse suit l'ETAGE, pas l'equipe. Sinon monter ses Pokemon
   ne sert a rien : les ennemis montaient avec eux et le taux de reussite
   restait plat quel que soit l'investissement. */
function expEnemyLevel(run, lvBonus){
  return clamp(10 + Math.round(run.floor * 4.1) + (lvBonus||0), 3, 100);
}
function enemyTeamFor(run, kind){
  const n = kind === "boss" ? 2 : (kind === "elite" || kind === "trainer") ? 2 : (run.floor > 8 ? 2 : 1);
  const lvBonus = kind === "boss" ? 2 : kind === "elite" ? 2 : 0;
  const boost = (kind === "boss" ? 1.13 : kind === "elite" ? 1.10 : kind === "trainer" ? 1.05 : 1)
              * (1 + run.corruption*0.06) * (cycleMod("m_tough") ? 1.3 : 1);
  const tr = kind === "trainer" ? trainerFor(run) : null;
  const out = [];
  for(let k=0;k<n;k++){
    let id;
    if(kind === "boss"){
      /* le noyau tire dans le haut de la fourchette finale */
      const [lo, hi] = bstBand(EXP_ROWS - 1);
      const pool = [];
      for(const r of unlockedRegions())
        for(let j=r.from;j<=r.to;j++)
          if(POKE[j] && !POKE[j].leg && !isExclusive(j) && POKE[j].bst >= Math.max(lo, 460) && POKE[j].bst <= 560) pool.push(j);
      id = pool.length ? pick(pool) : wildIdFor(run.floor);
    } else if(kind === "trainer"){
      id = wildIdFor(run.floor, {type: tr.t});
    } else if(kind === "elite"){
      /* l'Elite tire dans le haut de sa fourchette, pas au-dela */
      const [lo, hi] = bstBand(run.floor);
      const mid = lo + (hi - lo) * 0.55;
      const pool = [];
      for(const r of unlockedRegions())
        for(let j=r.from;j<=r.to;j++)
          if(POKE[j] && !POKE[j].leg && !isExclusive(j) && POKE[j].bst >= mid && POKE[j].bst <= hi) pool.push(j);
      id = pool.length ? pick(pool) : wildIdFor(run.floor);
    } else {
      id = wildIdFor(run.floor);
    }
    out.push(makeFighter(id, expEnemyLevel(run, lvBonus), {boost}));
  }
  return out;
}
...
function expFight(run, kind){
  ...
  createBattle(allies, foes, {
    ...
    onEnd: win => {
      const before = run.team.filter(m=>m.hp > 0).length;
      for(const f of allies) if(f.ref){ f.ref.hp = Math.max(0, f.hp); }
      const lost = before - run.team.filter(m=>m.hp > 0).length;
      if(lost > 0){
        run.lost = (run.lost || 0) + lost;
        buzz([50,40,80]);
        toast(lost > 1 ? `${lost} membres désindexés — définitif` : "Un membre désindexé — définitif",
              "bad", "cross");
      }
...
function afterFightRewards(run, kind, beaten){
  ...
  const back = classicRecover(run);
  if(back) toast(back > 1 ? `${back} membres reviennent en ligne`
                          : "Un membre revient en ligne", "", "heart");
  healTeam(run, heal);
...
function expRest(run){
  ...
  sheet(`<div class="center">
    ...
    <div class="tiny muted" style="margin:6px 0">Une secteur où le bruit de fond retombe.
...
ACTIONS.exprest = d => {
  const run = S.expedition;
  if(d.k === "heal"){
    /* le dernier repos ne remet pas a neuf : le noyau doit tester le PARCOURS,
     pas seulement la composition d'equipe. Sans usure residuelle, l'issue
     devient binaire — on gagne toujours ou on perd toujours. */
  healTeam(run, relicVal(run,"rest") ? 1 : 0.62);
    toast("Équipe reconstituée", "", "heart");
  } else {
    if(rng() < 0.55){ const r = grantRelic(run); toast(r?"Relique : "+r.n:"Rien trouvé", "warn", "shard2"); }
    else toast("Rien dans cette secteur.", "bad", "cross");
  }
  save(); closeSheet(); go("expedition");
};
