...
/* une espece est prete a evoluer si une de ses voies est entierement payable */
function evoReady(id){
  const p = POKE[id];
  if(!p || !p.evo || !p.evo.length || !S.dex[id]) return false;
  return p.evo.some(ev=>canEvolve(id, ev).ok);
}
function evoReadyCount(region){
  const r = regionDef(region);
  let n = 0;
  for(let i = r.from; i <= r.to; i++) if(evoReady(i)) n++;
  return n;
}
...
function canEvolve(id, e){
  ...
  const fc = friendCheck(id, e);
  if(fc) return fc;
  if(!friendEvo(id, e.to) && en.lvl < e.lvl) return {ok:false, why:`Niveau ${e.lvl} requis (actuel ${en.lvl}).`};
...
SCREENS.dex = {
  ...
  html(){
    ...
    for(let i=r.from;i<=r.to;i++){
      ...
      if(DEX_FILTER.mode === "evo" && !(e && evoReady(i))) continue;
    ...
    return `
      ...
      <div class="row between" style="margin-bottom:5px">
        <span class="grow"></span>${infoBtn("rarete")}</div>
      <div class="chipbar">
        ${[["all","Tous"],["owned","Possédés"],["missing","Manquants"],["shiny","Chromatiques"],
           ["evo","Peut évoluer" + (evoReadyCount(DEX_FILTER.region) ? " · " + evoReadyCount(DEX_FILTER.region) : "")]]
...
SCREENS.shop = {
  ...
  balls(){
    return `${karpDealPanel()}<div class="list">${Object.entries(BALLS).filter(([,b])=>!b.hidden).map(([k,b])=>`
      <div class="shopitem">
  ...
  boosts(){
    return `
      ...
      ${boostStrip()}
      <div class="list">${Object.entries(BOOSTS).filter(([id])=>id !== "b_shiny").map(([id,b])=>`
...
const CHEST_LOOT = {
/* Les archives servent a la variete — Balls, baies, pierres, cartes — et non
   a fabriquer des Fragments. Ceux-ci restent le revenu des modules de savoir-faire :
   sonde, expedition, Poke-Poker, verrous. */
  1:[{balls:{super:5}},{balls:{hyper:2}},{berries:{framby:4}},{coins:400},{shards:4},{berries:{nigma:2}}],
  2:[{balls:{hyper:6}},{balls:{data:4}},{shards:10},{coins:1600},{cores:1},{berries:{micle:2}},
     {items:"potion"}],
  3:[{cores:4},{shards:70},{balls:{master:1}},{coins:9000},{stones:"random"},{card:true},{held:true},
     {egg:"e_rare"}],
  4:[{cores:12},{shards:240},{balls:{master:2}},{coins:60000},{card:true},{held:true},
...
function avatarPool(){
  ...
  /* une piece verrouillee ne revele ni son nom ni son sujet */
  for(const r of REGIONS) for(const g of r.guardians){
    ...
    out.push({k:"guard:"+g.id, id:g.id, n:g.name, g:"Verrous levés",
              locked:!beaten, why:"Un verrou de " + r.name + " tient encore"});
  }
  out.push({k:"pz", id:474, n:"Porygon-Z", g:"Prestige",
            locked:!S.flags.climax, why:"Le récit n'est pas allé à son terme"});
  out.push({k:"mn", id:0, n:"Sans index", g:"Prestige",
            locked:!(S.cards && S.cards["mn:0"]), why:"Quelque chose n'a pas encore été rencontré"});
  return out;
}
function avatarKey(){
...
SCREENS.profile = {
  html(){
/* Les effets etaient definis dans le catalogue mais jamais rendus.
   Chacun pose une couche animee derriere l'en-tete du profil. */
function fxCls(){
  const f = S.cos.fx;
...
function fxLayerHtml(){
  ...
  if(f === "fx_stream") return `<span class="fxl fx-stream">
    ${Array.from({length:14},(_,i)=>`<i style="--x:${(i*7.4+3).toFixed(1)}%;--d:${(i%5)*0.7}s;--s:${(2.4+(i%4)*0.6)}s"></i>`).join("")}
  </span>`;
  if(f === "fx_cards")  return `<span class="fxl fx-cards">
    ${Array.from({length:7},(_,i)=>`<i style="--x:${(i*14+6)}%;--d:${i*0.55}s;--r:${(i%2?1:-1)*18}deg"></i>`).join("")}
  </span>`;
  return "";
}
    ...
    const pct = S.xp / need;
    return `
      <div class="hero ${fxCls()}" style="background:${bg.css||"linear-gradient(180deg,var(--s2),var(--s1))"}">
        ${fxLayerHtml()}
        ...
        <div class="row">
          ...
          <div class="grow">
            ...
            ${best?`<div class="tiny" style="color:${CARD_SERIES_DEF[best].c};margin-top:3px">
              Meilleure série obtenue : ${esc(seriesName(best))}</div>`:""}
          </div>
        </div>
      </div>
...
ACTIONS.toggleset = d => {
  if(d.k === "admin" && !S.settings.admin){ ACTIONS.adminpass(); return; }
      ...
      <div class="avgrid">
        ${list.slice(0, 60).map(a2=>`
          <div class="avcell ${cur===a2.k?"on":""} ${a2.locked?"locked":""}"
            ${a2.locked?`data-act="avlocked" data-w="${esc(a2.why)}"`:`data-act="setavatar" data-k="${a2.k}"`}
            title="${esc(a2.locked ? a2.why : a2.n)}">
            ${a2.locked
              ? `<span class="avunknown">?</span>`
            ...
            <span class="avnm">${a2.locked ? "?????" : esc(a2.n)}</span>
          </div>`).join("")}
      </div>`).join("")}`);
};
ACTIONS.avlocked = d => toast(d.w || "Pièce non débloquée", "bad", "lock");
