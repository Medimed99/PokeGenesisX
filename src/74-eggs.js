...
const EGG_TIERS = {
  ...
  e_epic:   {n:"Œuf Épique",     steps:60,  rar:[2,3,4],   shinyMul:6,   cores:9,  c:"#b06bff",
             d:"Une espèce rare à altérée. Chance de chromatique multipliée par six."},
  ...
  /* le fossile n'est pas un oeuf : c'est une definition ancienne, deterree
     par le forage, que la couveuse sait reconstituer */
  f_fossil: {n:"Fossile",         steps:25,  rar:[],        shinyMul:2,   cores:0,  c:"#c9a86a",
             d:"Une définition très ancienne, déterrée par le forage. On ne sait pas ce qu'elle contient avant de la reconstituer.",
             fossil:1}
};
/* les fossiles de base, par secteur : leurs evolutions viennent en les faisant evoluer */
const FOSSILS = {kanto:[138, 140, 142], hoenn:[345, 347]};
const EGG_SHOP_LEVEL = 20;          /* avant ce niveau, les oeufs se meritent */
...
function grantEgg(tier){
  ...
  /* premiere trouvaille : c'est elle qui ouvre la couveuse, pas un palier de niveau */
  if(!S.flags.eggFound){
    S.flags.eggFound = true;
    S.flags.eggReveal = tier;
  }
  saveSoon();
  return tier;
}

/* ---------- découverte du premier œuf ---------- */
function showEggDiscovery(tier){
  const t = EGG_TIERS[tier] || EGG_TIERS.e_common;
  const box = document.getElementById("discover");
  if(!box){ toast("Module Couveuse débloqué", "warn", "box"); return false; }
  box.className = "on eggfind";
  box.style.setProperty("--ac", t.c);
  box.innerHTML = `
    <div class="dc-rays"></div>
    <div class="dc-inner">
      <div class="dc-kicker" id="ef-kick">OBJET NON RÉPERTORIÉ</div>
      <div class="hatch-stage">
        <div class="egg" id="ef-egg" style="--ec:${t.c}"><i></i><i></i><i></i></div>
      </div>
      <div class="dc-name in" style="font-size:12px">${esc(t.n)}</div>
      <div class="dc-lore in" id="ef-lore">
        Ce n'est pas une entité archivée. C'est une définition que le système a commencé
        à écrire, puis abandonnée.</div>
      <div class="panel tight" style="text-align:left;margin:0 0 10px">
        <div class="h sm">MODULE OUVERT — COUVEUSE</div>
        <div class="tiny muted">Une ébauche se termine d'elle-même quand quelqu'un travaille
          à côté. Placez-la en incubation : elle avancera d'un pas à chaque capture.</div>
      </div>
      <button class="btn pri wide dc-ok in" data-act="efdone">Ouvrir la couveuse</button>
    </div>`;
  const egg = document.getElementById("ef-egg");
  [260, 700, 1140].forEach(ms=>setTimeout(()=>{
    if(!egg) return;
    egg.classList.remove("crack"); void egg.offsetWidth; egg.classList.add("crack");
    Sfx.wobble(); buzz(12);
  }, ms));
  setTimeout(()=>{
    const k = document.getElementById("ef-kick");
    if(k){ k.textContent = "DÉCOUVERTE"; k.classList.add("done"); }
    const st = document.querySelector("#discover .hatch-stage");
    if(st) burstEl(st, {n:22, spread:130, colors:[t.c, "#ffffff"], dur:900});
    Sfx.win();
  }, 1450);
  return true;
}
...
function eggPool(tier){
  ...
  const out = [];
  if(t.fossil){
    for(const r of unlockedRegions()) for(const id of (FOSSILS[r.key] || [])) out.push(id);
    return out.length ? out : [138, 140];
  }
  for(const r of unlockedRegions())
    for(let i = r.from; i <= r.to; i++){
      const p = POKE[i];
      if(!p) continue;
      const baby = EXCL_SOURCE[i] === "egg";
      if(!t.rar.includes(p.rar) && !(baby && tier !== "e_origin")) continue;
      if(p.leg && tier !== "e_origin") continue;        /* les légendaires sont réservés */
      /* aucune exclusivite de peche ou de forage dans un oeuf */
      if(EXCL_SOURCE[i] && EXCL_SOURCE[i] !== "egg") continue;
      /* les bebes, eux, ne naissent QUE d'un oeuf : ils y sont favorises */
      if(EXCL_SOURCE[i] === "egg"){ for(let k = 0; k < 5; k++) out.push(i); continue; }
...
SCREENS.eggs = {
  ...
  html(){
    ...
    return `
      ...
      ${S.level < EGG_SHOP_LEVEL
        ? `<div class="empty">Le tampon de rebut ne vend rien à un Archiviste de niveau
            ${S.level}. Revenez au niveau ${EGG_SHOP_LEVEL} — d'ici là, les ébauches se
            trouvent en expédition.</div>`
        : `<div class="list">
...
/* ============================================================
   EXTRACTION D'UNE ÉBAUCHE
   Un oeuf ramene du noyau ne se lit pas dans une liste. On le sort
   de la couche, on l'analyse, et le systeme finit par avouer ce
   qu'il contient — ou refuse de le dire.
   ============================================================ */
let EGGFIND = null;

function showEggFind(tier, onDone){
  const t = EGG_TIERS[tier] || EGG_TIERS.e_common;
  const box = document.getElementById("discover");
  if(!box){ if(onDone) onDone(); return false; }
  const rank = Object.keys(EGG_TIERS).indexOf(tier);

  EGGFIND = {onDone, timers:[]};
  box.className = "on eggfind";
  box.style.setProperty("--ac", t.c);
  box.innerHTML = `
    <div class="bo-rays"></div>
    <div class="dc-inner">
      <div class="dc-kicker" id="eg-kick">EXTRACTION EN COURS</div>

      <div class="eg-stage" id="eg-stage">
        <div class="eg-capsule" id="eg-caps">
          <div class="eg-glass"></div>
          <div class="egg" style="--ec:${t.c}"><i></i><i></i><i></i></div>
          <div class="eg-scan" id="eg-scan"></div>
          <div class="eg-ticks"><i></i><i></i><i></i><i></i></div>
        </div>
        <div class="eg-plinth"></div>
      </div>

      <div class="eg-readout" id="eg-read">
        <div class="eg-line"><span>SOURCE</span><b>couche non cartographiée</b></div>
        <div class="eg-line"><span>ÉTAT</span><b class="eg-wait">analyse…</b></div>
        <div class="eg-line"><span>CLASSE</span><b class="eg-wait">—</b></div>
      </div>

      <div class="dc-lore in" id="eg-lore">Ce n'est pas une entité archivée.</div>
      <button class="btn pri wide dc-ok" id="eg-ok" data-act="egfdone">Emporter</button>
    </div>`;

  const at = (ms, fn) => EGGFIND.timers.push(setTimeout(fn, ms));
  const caps = document.getElementById("eg-caps");
  const scan = document.getElementById("eg-scan");

  at(120, ()=>{ if(caps) caps.classList.add("in"); Sfx.click(); });
  at(700, ()=>{ if(scan) scan.classList.add("on"); Sfx.wobble(); });
  /* trois passes de lecture, de plus en plus rapides */
  [1050, 1500, 1880].forEach((ms, i)=>at(ms, ()=>{
    const el = document.querySelector("#eg-read .eg-line:nth-child(2) b");
    if(el) el.textContent = ["lecture 1/3…", "lecture 2/3…", "définition incomplète"][i];
    Sfx.click(); buzz(8);
  }));

  at(2280, ()=>{
    if(scan) scan.classList.remove("on");
    if(caps) caps.classList.add("done");
    const k = document.getElementById("eg-kick");
    if(k){ k.textContent = "ÉBAUCHE RÉCUPÉRÉE"; k.classList.add("done"); }
    const cls = document.querySelector("#eg-read .eg-line:nth-child(3) b");
    if(cls){ cls.textContent = t.n; cls.className = "eg-hit"; cls.style.color = t.c; }
    const st = document.getElementById("eg-stage");
    if(st) burstEl(st, {n: 16 + rank*8, spread:140, colors:[t.c, "#ffffff"], dur:950});
    const lo = document.getElementById("eg-lore");
    if(lo) lo.textContent = t.d;
    const ok = document.getElementById("eg-ok");
    if(ok) ok.classList.add("in");
    rank >= 2 ? Sfx.win() : Sfx.shiny();
    buzz(rank >= 2 ? [25,45,80] : [18,34]);
  });
  return true;
}
ACTIONS.egfdone = () => {
  const done = EGGFIND && EGGFIND.onDone;
  if(EGGFIND){ EGGFIND.timers.forEach(clearTimeout); EGGFIND = null; }
  const box = document.getElementById("discover");
  if(box){ box.className = ""; box.innerHTML = ""; }
  if(done) done();
};
