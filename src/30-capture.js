...
function featuredId(){
  ...
  for(const r of unlockedRegions())
    for(let i=r.from;i<=r.to;i++){
      const p = POKE[i];
      if(p && p.leg === 0 && p.rar <= 3) pools[p.rar].push(i);
    }
...
function rollRarity(){
  ...
  const boostRare = eventActive("ev_rare") || boostActive("rare") || luckyF("rare") > 0;
  const rareMul = 4.5 * (boostActive("rare") ? 1.6 : 1) * (1 + luckyF("rare"));
...
function speciesForRarity(rar){
  ...
  if(!out.length){
    for(let i=r.from;i<=r.to;i++) if(POKE[i] && POKE[i].leg===0) out.push(i);
  }
  return pick(out);
}
...
function newEncounter(force){
  ...
  else { rar = force?.rar ?? rollRarity(); id = force?.id ?? speciesForRarity(rar); rar = POKE[id].rar; }
  ...
  if(!force && boostActive("new") && rng() < 0.2){
    ...
    for(let i=rg.from;i<=rg.to;i++) if(POKE[i] && !S.dex[i] && POKE[i].leg===0) miss.push(i);
  ...
  if(shiny){ Sfx.shiny(); buzz([18,40,18,40,30]); pzFlash(pzLine("shiny")); }
...
/* --- visée : mini-jeu d'adresse ---
   ...
   Rater la zone rend exactement la chance nominale, viser juste la majore
...
function startAim(){
  if(AIM) return;
  const w = 30 - Math.min(16, ENC.rar * 4);              /* zone plus etroite si rare */
...
function onCatch(ballKey){
  ...
  coins = Math.round(coins * (1 + Math.min(0.6, S.streak*0.004))    /* appoint, pas jackpot */
                            ...
                            * perkMul("p_coin", 0.15) * (cycleMod("m_econ") ? 0.5 : 1));
...
function onFail(ballKey){
  ...
  if(rng() < fleeChance()){
    ...
    if(safe){
      toast("Baie Nanab : série préservée", "warn", "check");
    } else if(S.streak > 0){
      breakStreak();
    }
    ...
    ENC = null;
    setTimeout(()=>{ newEncounter(); refresh(); }, 420);
  } else refresh();
  checkAchievements();
  guideTick();
  saveSoon();
...
function resolveMissing(ballKey){
  ...
  setTimeout(()=>{
    ...
    save();
    if(!checkStoryTriggers()){ newEncounter(); refresh(); }
  }, 900);
}
...
const STREAK_TIERS = [
  {n:10,  rw:{coins:250},                 t:"Série 10 — la chaîne tient."},
  {n:25,  rw:{shards:12},                 t:"Série 25 — flux stable."},
  {n:50,  rw:{balls:{super:5}},           t:"Série 50 — le secteur se lit tout seul."},
  {n:75,  rw:{berries:{micle:1}},         t:"Série 75 — instabilité chromatique offerte."},
  {n:100, rw:{cores:1, balls:{hyper:5}},  t:"Série 100 — flux ininterrompu."},
  {n:150, rw:{cores:2},                   t:"Série 150 — je n'avais jamais vu ça."},
  {n:250, rw:{cores:3, shards:60},        t:"Série 250 — tu forces le système à être cohérent."}
];
...
ACTIONS.nextenc = () => { closeSheet(); if(!checkStoryTriggers()){ newEncounter(); refresh(); } };
...
function selBall(){
  ...
  return BALLS[k] ? k : "poke";
}

SCREENS.capture = {
  html(){
    if(!ENC && !busy) newEncounter();
    ...
    <div class="chipbar">
      ...
      ${locked.map(r=>{
        ...
        return `<button class="chip locked" data-act="regioninfo" data-r="${r.key}">
          ${ic("lock")} ${esc(r.name)}
          <em>${esc(g.core.name)}</em></button>`;}).join("")}
    </div>
    ...
    <div class="streakbox" style="margin-bottom:9px">
      <span class="tiny dim">CHAÎNE</span>${infoBtn("streak")}
...
ACTIONS.regioninfo = d => {
  ...
  sheet(`${sheetHead("Secteur " + r.name + " — verrouillé")}
    ...
    <div class="panel tight">
      ...
      <div class="tiny">Relâcher <b class="bad">${esc(g.core.name)}</b>, le noyau du secteur
        ${esc(g.prev.name)}.</div>
    </div>
    <div class="panel tight">
      ...
      ${g.prev.guardians.map((gd,i)=>{
        ...
        return `<div class="row between tiny" style="padding:3px 0">
          <span class="${beaten?"ok":av?"":"dim"}">${beaten?"✓":"·"} ${esc(gd.name)}</span>
