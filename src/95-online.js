...
const Net = {
  ...
  async leaderboard(metric, limit){
    const cols = "name,level,integrity,dex_count,shinies,best_streak,best_ante,guardians,cards,tested,user_id";
...
SCREENS.online = {
  ...
  board(){
    const metrics = [["integrity","Intégrité"],["dex_count","Pokédex"],["level","Niveau"],
                     ["shinies","Chromatiques"],["best_streak","Série"],["best_ante","Poké-Poker"],
                     ["guardians","Verrous"]];
    return `
      <div class="chipbar">
          ...
          : NET_BOARD.length === 0
            ...
            : `<div class="list">${NET_BOARD.map((r,i)=>{
                ...
                const val = {integrity:r.integrity+"%", dex_count:r.dex_count+"/386", level:"niv."+r.level,
                  ...
                  guardians:r.guardians+"/9"}[NET_BOARD_METRIC];
              ...
              return `<div class="shopitem">
                ...
                <div class="grow"><div>${esc(nm)}</div>
                  <div class="tiny" style="color:${CARD_SERIES_DEF[sr].c}">
                    ${esc(CARD_SERIES_DEF[sr].n)} · ${e.dup} en double</div></div>
