...
function sprite(id, shiny, cls, opts){
  ...
  const shinyCls = shiny ? (wantAnim ? "shinyglow" : "shiny") : "";
...
function storyPortrait(w, sceneArt, line){
  if(w === "pz")   return pzFace(pzMood(line), 132, "big");
  ...
  document.getElementById("topbar").innerHTML = `
    <div class="tb-row">
      <div class="tb-id">
        ...
        <div class="tb-sub">Archiviste niv.${S.level} · ${Math.floor(S.xp/need*100)}% XP</div>
      </div>
      <div class="tb-cur">
    ...
    <div class="integrity">
      <span class="lbl">INTÉGRITÉ</span>${infoBtn("integrity")}
...
function closeSheet(){ const r = document.getElementById("sheet-root"); if(!r) return; r.className=""; r.innerHTML=""; }
...
function playStory(id, onDone){
  ...
  if(!S.story.includes(id)) S.story.push(id);
