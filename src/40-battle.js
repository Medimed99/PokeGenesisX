...
function doTurn(moveIdx, forcedSwitch){
  ...
  const allyFirst = !forcedSwitch &&
    (A.spe * A.buff.spe * (A.status && A.status.k === "par" ? .5 : 1)) >=
    (F.spe * F.buff.spe * (F.status && F.status.k === "par" ? .5 : 1));
  const seq = [];
...
function applyMove(side, att, def, mv){
  ...
  const {dmg, eff} = damageCalc(att, def, mv, crit);
  def.hp = Math.max(0, def.hp - dmg);
  Sfx.hit(); if(side === "f") buzz(14);
  blog(`${esc(att.name)} utilise <b class="cy">${esc(mv.name)}</b> — <b>${dmg}</b>
...
function renderArena(){
  ...
  host.innerHTML = `
    ...
    ${BATTLE.over ? "" : `<div class="turnflag ${BATTLE.busy?"wait":""}">
          ...
          <div class="list" style="margin-top:7px">
            ${r.guardians.map((g,i)=>{
              ...
              return `<div class="item ${beaten?"":""}" style="${beaten?"border-color:var(--green)":av?"border-color:var(--magenta)":""}">
                ${sprite(g.id, false, "sm", beaten?"":'style="filter:brightness(.4) contrast(1.4) hue-rotate(300deg)"')}
                <div class="grow">
                  <div class="t">${esc(g.name)}</div>
                  <div class="d">${esc(g.title)} · Niv.${guardianLevel(r,i)}</div>
...
function onGuardianEnd(r, i, win){
  ...
  if(win){
    ...
    if(first) S.bosses.push(guardianKey(r.key, g.id));
    S.stats.bossWins++;
    ...
    sheet(`<div class="center">
      ...
      ${first?`<div class="panel" style="text-align:left">
        ...
        <div class="tiny">Carte émise : <b style="color:${CARD_SERIES_DEF[cs].c}">${esc(CARD_SERIES_DEF[cs].n)}</b>
