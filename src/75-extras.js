...
const WEEKLY_POOL = [
  ...
  {id:"w_fish",  n:"Réussir {n} sondages",               goal:[25,40,60],    stat:"wkFish"},
  {id:"w_energy",n:"Produire {n} d'Énergie Onirique",    goal:[25000,60000,120000], stat:"wkEnergy"}
];
...
function rollWeekly(){
  ...
  S.weekly = {
    week: weekKey(),
    tier,
    quests: shuffle(WEEKLY_POOL).slice(0,3).map(q=>({
...
ACTIONS.claimwk = d => {
  ...
  revealSheet("OBJECTIF HEBDOMADAIRE", [
    ...
    {icon:"cards", label:POKE[wc.id].name,
     sub:"carte " + CARD_SERIES_DEF[wc.s].n + (wcNew ? "" : " · doublon"),
...
function revealSheet(title, items, opts){
  ...
  sheet(`<div class="center reveal">
    ...
    <button class="btn pri wide" id="rv-ok" style="opacity:0;pointer-events:none"
      data-act="closeandrefresh">Récupérer</button>
  </div>`, true);
  runReveal(items);
}
...
function openChest(key, silent){
  ...
  for(let i=0;i<n;i++){
    ...
    if(loot.card){
      ...
      out.push({id:c2.id, label:POKE[c2.id].name,
                sub:CARD_SERIES_DEF[c2.s].n + (fresh ? "" : " · doublon"),
                color:CARD_SERIES_DEF[c2.s].c, big:true});
      delete loot.card;
    }
