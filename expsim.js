...
eval(ORDER.map(f=>fs.readFileSync("src/"+f,"utf8")).join("\n") +
 ...
 "newRunMon:newRunMon,randomWildId:randomWildId,relicVal:relicVal," +
...
function setup(lvl){
  ...
  S.team=G.ownedSorted().slice(0,4);
  return S;
}
...
function runOnce(lvl){
  const S=setup(lvl);
  G.startExpedition(S.team.slice(0,4));
  ...
  while(y<G.EXP_ROWS){
    ...
    if(k==="fight"||k==="elite"||k==="boss"){
      ...
      if(k!=="boss" && run.team.length<6 && Math.random()<0.6)
        run.team.push(G.newRunMon(run, G.randomWildId(), run.level));
    } else if(k==="rest"){ G.healTeam(run, G.relicVal(run,"rest") ? 1 : .55); }
...
for(const lvl of [25, 40, 55, 70]){
  const agg={fight:{w:0,l:0}, elite:{w:0,l:0}, boss:{w:0,l:0}};
  ...
  console.log(`équipe niv.${String(lvl).padStart(2)} | combat ${pct(agg.fight).padStart(4)}`
    +` | élite ${pct(agg.elite).padStart(4)} | noyau ${pct(agg.boss).padStart(4)}`
