...
function run(label, ballKey, lvl){
  G.setS(G.newState()); G.dailyRollover();
  const S=G.getS(); S.level=lvl; S.coins=0; S.event=null; S.balls[ballKey]=1e9;
  ...
  for(let i=0;i<4000;i++){
    ...
    while(!done && guard++<8){
      ...
      S.level=lvl; S.event=null;   /* niveau et événements figés : on mesure la boucle seule */
    }
  }
...
rateTable();
console.log("\nLONGUEUR DE SÉRIE (3000 rencontres)");
for(const b of ["poke","super"]){ const r=streakSim(b,3000);
